...
respond_to_group_messages = true   # Respond/forward in groups; requires listen_group_messages
```
...
| `chat.respond_to_group_messages` | `true` | Respond to group triggers (@bot, etc.) and forward tracked Alma GUI replies back to QQ groups. Requires `listen_group_messages`; when listen is off, respond is forced off |
